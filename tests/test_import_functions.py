...
class TestPythonOfficialImport:
    ...
    @patch("import_python_official.request.urlopen")
    def test_ics_parsing(self, mock_urlopen):
...
END:VCALENDAR"""
        ...
        mock_file = Mock()
        mock_file.read.return_value = mock_ics_content
...
class TestPythonOrganizersImport:
    ...
    def test_column_mapping(self):
        ...
        # Create sample CSV-style DataFrame
        csv_df = pd.DataFrame(
            {
                "Name": ["PyCon Test"],
                "Year": [2025],
        ...
        expected_columns = ["conference", "year", "link", "cfp", "place", "start", "end"]
        for col in expected_columns:
            assert col in mapped_df.columns or any(col in str(c).lower() for c in mapped_df.columns)
    ...
    def test_main_function(self, mock_write, mock_load):
        ...
        # Should load data for current and next year
